<script lang="ts">
  ...
  import { FadeIn, SlideDown } from 'svelte/transition';
  ...
  function handleNavClick(href: string) {
    ...
    dispatch('nav-click', { href });
  }
</script>

<nav aria-label="Primary Navigation" class:glass:class="fixed top-0 left-0 right-0 z-50 bg-white/90 dark:bg-dark-800/90 backdrop-blur-md border-b border-gray-200 dark:border-dark-600 transition-all duration-300"
  class:shadow-md={scrolled}>
  <div class="max-w-7xl mx-auto px-4 md:px-8 lg:px-16">
    <div class="flex items-center justify-between h-16 md:h-20">
  <!-- Logo -->
  <a href="/" class="flex items-center gap-2 group" on:click|preventDefault={() => handleNavClick('/')} aria-label="Randell Construction Home">
        ...
        <div class="hidden sm:block">
          <span class="text-xl font-heading font-bold text-dark-900 dark:text-white">Randell</span>
          <span class="text-xl font-heading font-bold text-primary-500"> Construction</span>
        </div>
      </a>
      ...
      <div class="flex items-center gap-4">
        ...
        <a href="/request-quote"
           ...
           on:click|preventDefault={() => handleNavClick('/request-quote')}>
